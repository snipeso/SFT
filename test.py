...
msg2 = visual.TextStim(
    win, "Close plot window to continue", color=-1, height=0.05)
circle.draw()
instr.draw()
win.flip()
...
for i in range(10):
    ...
    filename = mic.record(rec_duration)
    ...
    # increase secs if miss the markers
    onset, offset = mic.getMarkerOnset(chunk=64, secs=0.2)
    onsets.append(onset)
    ...
    if len(event.getKeys()):
        ...
        plotYX(mag, freqV, "frequency domain (marker at %i Hz)" %
               mic.getMarkerInfo()[0])
